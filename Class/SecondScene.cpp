...
static SecondScene *m_mainscene = NULL;
//创建物理世界层
Scene* SecondScene::createScene()
{
	...
	m_mainscene = SecondScene::create();
...
bool SecondScene::init()
{
	...
	extern int a;
	a = 2;
	FlyWord *word = FlyWord::create(5,Vec2(384,512));
	addChild(word,9);
	...
	//添加我方英雄飞机
	{
		heroPlaneG = HeroPlane::createHeroPlane("plane1.png");
	...
	/*主要对象的生成 包括飞机和子弹*/
	{
		...
		//定时随机产生Buff道具
		schedule(schedule_selector(SecondScene::manageBuff), 4.0f);
		//定时产生我方子弹并移动
		schedule(schedule_selector(SecondScene::mangerMainBullet), 0.2f);
		//定时移动子弹
		schedule(schedule_selector(SecondScene::moveBullet), 0.005f);
		//高频率判断是否游戏结束
		schedule(schedule_selector(SecondScene::judgeWhetherGameIsOver), 0.01f);
	}
	/*轮播背景 设置按钮*/
	{
		//背景图一
...
void SecondScene::mangerMainBullet(float time) {
	...
	else
	{
		...
		{
			blt1 = Bullet::createBullet("bullet4.png");
			Vec2 planeLocal = heroPlaneG->getPosition();
			blt1->type = bullet_main;
...
//移除道具
void SecondScene::removeBuff(Ref *object) {
...
//持续随机生成道具 简易算法
void SecondScene::manageBuff(float time) {
	int buffNum = 0;
	if (buffNum < 1000) {
		buffNum++;
...
//血量
void SecondScene::refreshTheHP() {
	CallFunc* HPtextShow = CallFunc::create(CC_CALLBACK_0(SecondScene::changeTheHP, this));
...
//改变分数
void SecondScene::changeTheScore() {
	char ScoreFormat[20];
...
//加入定时器刷新分数
void SecondScene::refreshTheScore() {
	changeTheScore();
}
...
//判断主机是否死亡
void SecondScene::judgeWhetherGameIsOver(float time) {
	...
	if (IsBossDead == true) {
		...
		/*CallFuncN *fun = CallFuncN::create(CC_CALLBACK_0(MainGame::victory, this));
