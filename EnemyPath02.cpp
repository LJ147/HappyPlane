...
void EnemyPath::chapter1() {
	...
	Sequence* seq = Sequence::create( fun0,fun1, fun7,
		fun8,
		DelayTime::create(24),
		fun2,
		DelayTime::create(29),
...
void EnemyPath::MangerBoss() {
	...
	//mangerBossBullet();
	schedule(schedule_selector(EnemyPath::RunBullet1), 12);

	schedule(schedule_selector(EnemyPath::RunBullet2), 12, 999, 3);
...
void EnemyPath::BossBullet2(float rad,int tag) {
	SpriteFrameCache::getInstance()->addSpriteFramesWithFile("instancePictures.plist", "instancePictures.png");
	Bullet *blt2;
	if (tag == 1) {
		blt2 = Bullet::createBullet("bullet4.png");
	}
	else if (tag == 2) {
		blt2 = Bullet::createBullet("bullet3.png");
	}
	...
	blt2->setRotation(rad*180/PI);
	float endX2;
	float endY2;
	float flyLen2;
	float flyVelocity = 200;
	if (blt2->getPosition().y > blt2->getPosition().x / tan(rad)) {
		endX2 = 0;
		endY2 = blt2->getPosition().y - blt2->getPosition().x / tan(rad);
		flyLen2 = blt2->getPosition().x / sin(rad);
	}
	else {
		endX2 = blt2->getPosition().x - blt2->getPosition().y*tan(rad);
		endY2 = 0;
		flyLen2 = blt2->getPosition().y / cos(rad);
	}
...
void EnemyPath::RunBullet1(float time) {
	this->BossBullet2(0.3926991,1);
	this->BossBullet2(0.7853982,1);
	this->BossBullet2(1.5707964,1);
	this->BossBullet2(1.9634955,1);
	this->BossBullet2(2.3561946,1);
}

void EnemyPath::RunBullet2(float time) {
	this->BossBullet2(0.3926991, 2);
	this->BossBullet2(0.7853982, 2);
	this->BossBullet2(1.5707964, 2);
	this->BossBullet2(1.9634955, 2);
	this->BossBullet2(2.3561946, 2);
}